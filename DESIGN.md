...
`prose.analyse` takes a table of inline parses keyed by range and returns the
gaps that are **breakable**; everything else stays source-exact inside an atom.
Two separate rules remove a gap, and they are easy to confuse:

- **An admitted construct is kept whole** by dropping the gaps *inside* it, so
  a code span, link or autolink becomes one atom. The gaps on either side of it
  stay breakable -- in ``alpha `beta` gamma`` the breakable offsets are 5 and
  12, both flanks.
- **A block-hazardous atom is protected bilaterally**, both flanks dropped. In
  `alpha -- beta gamma` only offset 13 survives: binding just the predecessor
  lets an over-width atom force the *following* gap to break, and the next
  source line then reads as a setext underline or a table delimiter row.

Emphasis, non-ASCII atom content, escapes and containers are still refused, at
A2.2 through A2.4.
