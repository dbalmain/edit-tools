...
| 19  | Markdown   | Opus (orchestrator) | `table` opcode — lay a node out as an aligned pipe table, padding cells to their column widths and redrawing the ruler row to match | A column's width is the widest cell in it and the ruler under that column has to be drawn to the same width, so `:-` becomes `:-----`. No pretty-printer expression can do that: the DSL chooses where breaks go and may not invent token text. The alternative measured was `cell`/`cellblock`, which pads to a per-column max but is left-align only, cannot right- or centre-align, and has run-detection heuristics (a blank line or a `}` ends a run) that are wrong for a table. It dissolves three defects at once: a cell with inline markup refused, a table in a list or quote refused, and the padding itself. +1231 B gzip, 725 B of it the prose explaining the rewrite. | **unreviewed.** Built by Claude (Opus 5) 2026-09-09, `7b5bf96`. Five tests each side. |
| 20  | Markdown   | Opus (orchestrator) | `layout_leaves` — a manifest list of leaf kinds whose text gate 3 compares by meaning rather than byte for byte                    | Row 19's padding rewrites a leaf, and gate 3 rejected **prettier's own output** on `tables.md` for it: `('-','-','-')` became eleven dashes, `'a '` became `'a   '`. The gate's own docstring says a rejection of the reference formatter means the gate is wrong rather than strict — it is the same failure black's added parentheses produced, and takes the same remedy, a narrowing declared per language by kind. Outer whitespace goes; a leaf that is entirely a rule reduces to its anchors, so `:-` equals `:-----` and does not equal `-----:`. What it stops catching, stated in the code: a body cell whose content is `--` becoming `---`. | **unreviewed.** Built by Claude (Opus 5) 2026-09-09, `7b5bf96`. Four unit tests; `check_gate3` still rejects 782 destructive mutations, markdown's own share up from 32 to 34. |
...
| 23  | Markdown | Codex | `whitespace_nodes` package and manifest declarations; package format 2, with version 1 still supported | FINDINGS 36's empty `section` is a literal whitespace leaf as well as an item. Consume declared whitespace-only leaves as source gaps before comment attachment; the next real item receives one separator and leading trivia none. The subtree must pass source/range validation; meaningful leaves, interior nodes and injected roots remain items. No Doc inspection, tree rewriting or new opcode earned a place: 29 opcodes and three sanctioned mutations remain. The manifest declaration is separately required because gate 3 rejected Prettier's own removal of a leading section. Added front matter to `sections.md` and a three-blank leading run in `leading_sections.md`; both explicitly double-format identically in both runtimes at 80 and 40 and match Prettier. | **unreviewed.** Built by Codex, 2026-09-09. Six mirrored runtime tests, structural and manifest safeguards; all 792 destructive mutations still rejected (782 before the session). JS runtime +369 B gzip; markdown package +19 B. |
