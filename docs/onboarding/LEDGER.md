...
| 12  | JSON/CSS | codex-Sol       | `fill`: pack alternating content and separators independently per line                                                    | JSON packs a long primitive array where group/each emitted one item per line; CSS stage D found fill contributes to 13/21 divergences and fully resolves 9/21. The printer already owns the remaining width, so this stays local: no sibling measurement or ancestor state.                                                                                       | warranted; `matrix` is excluded because it needs the separately deferred all-child-kind predicate | +365 B | 2026-08-17 |
...
against the 20 KB soft budget. The runtime grew 8,080 → 10,315 across the
...
two-content fit decision. No existing package opted in at this boundary, and
all existing outputs remained byte-identical.
