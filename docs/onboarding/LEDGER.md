...
| 14  | Rust       | grok                | comment text recovered from the source range when a comment node carries no `text`                                                                    | tree-sitter-rust's `line_comment`/`block_comment` are **interior** nodes — the `//` is a child and the body lives only in the span — while `gen_trees.py` writes `text` on leaves only, so attach emitted empty comment suffixes. A general runtime defect exposed by Rust, not a Rust feature; same class as row 5.                                                                                                                                                                                                                          | warranted (orchestrator merge review). Cherry-picked to `main` ahead of the Rust slice because a second builder was independently converging on the same bug and would have produced a second design for it.                                                                                                                                                                                                                                                                                                                    | +299 B                                                                                                                                                                                                                                                                      | 2026-08-19                                                                                                                                         |
| 15  | JavaScript | DeepSeek            | `srcbreak` opcode — a break that is `hard` when the source broke and a group `line` when it did not                                                   | prettier's `objectWrap: preserve` needs both behaviours at once. `srcline`/`srcsoft` fall back to a space or nothing, which **ignore the enclosing group**, so a source-flat object that must break by width strands its first property. No composition reaches it: branching on source-brokenness would need a predicate, and `count`/`child-count`/`all` cannot see line breaks.                                                                                                                                                            | warranted (orchestrator merge review). Completes an existing family — one `srcBreak(flat)` helper, three call sites — rather than adding a mechanism. Cheapest runtime change on this table.                                                                                                                                                                                                                                                                                                                                    | +16 B                                                                                                                                                                                                                                                                       | **upheld** — JavaScript stage D reviewed all 15 divergences `design-limit`, verdict `merge`, and named no runtime edit as unnecessary (2026-08-19) |
| 16  | Rust       | grok                | optional leading fraction on `group`: break unless the flat form fits **both** the remaining columns and `round(max * width)` of the group's own span | FINDINGS 17. rustfmt carries nine width thresholds; a construct that fits the line still breaks if it exceeds its own. Unreachable by our single-width `group` **at any width**. Spike `spike/rust-subwidth` measured it end to end.                                                                                                                                                                                                                                                                                                          | warranted (orchestrator merge review). Verified inert when undeclared by re-scoring every language at 91/138 unchanged, not by reading the code. Cheapest of the three local capabilities: `fill` +365 B, alignment +2,627 B.                                                                                                                                                                                                                                                                                                   | +228 B                                                                                                                                                                                                                                                                      | 2026-08-19                                                                                                                                         |
| 17  | Rust       | Opus (orchestrator) | `comment_cells` gains a `"block"` scope, and the trailing-comment column becomes width-aware                                                          | FINDINGS 22. One package-wide switch cannot serve two languages that want opposite scopes: Kotlin wants every trailing comment in a column, Rust wants only the ones inside a `cellblock`. The header grew a third value rather than a second field, and the closers `}`/`)`/`]` stopped starting cells. The width half is rustfmt's own rule — a comment leaves its column when alignment would push the line past the printer width — pinned by running rustfmt at four widths, because two corpus files disagree under every simpler rule. | **kept, still unreviewed.** Built by the orchestrator, so no independent verdict exists. The scope half closed six `package-bug` divergences; the width half closed none, and Dave kept it anyway (2026-08-21) on the grounds that a comment overrunning the box is exactly the failure `house-style.md` names, and that file agreement cannot see it — an argument about the metric, not the output. If the budget ever forces a give-back, FINDINGS 22 records how to restore it and this is the cheapest 471 B on the table. | +958 B                                                                                                                                                                                                                                                                      | not merged (`worktree-feat-cell-scope`)                                                                                                            |
| 18  | Rust       | Opus (orchestrator) | `drop` opcode — consume a declared punctuation token without emitting it                                                                              | FINDINGS 13, whose stated trigger (a second language wanting it) fired when rustfmt's deletion of the redundant leading `|`made`leading_pipes.rs` incomparable. Guarded twice: the token must be declared punctuation, and it must carry no comment.                                                                                                                                                                                                                                                                                                                                                                                                      | **parked** (Dave, 2026-08-21). Correct, six unit tests, and used by no package — gate 3's two-branch skeleton rejects the one arm that would have earned agreement, and Go's candidate file needs entry 10 as well. Kept rather than reverted; `6a357bd` has no dependents. | +317 B                                                                                                                                             | not merged (`worktree-feat-cell-scope`) |

| 19 | Markdown | Opus (orchestrator) | `table` opcode — lay a node out as an
aligned pipe table, padding cells to their column widths and redrawing the ruler
row to match | A column's width is the widest cell in it and the ruler under
that column has to be drawn to the same width, so `:-` becomes `:-----`. No
pretty-printer expression can do that: the DSL chooses where breaks go and may
not invent token text. The alternative measured was `cell`/`cellblock`, which
pads to a per-column max but is left-align only, cannot right- or centre-align,
and has run-detection heuristics (a blank line or a `}` ends a run) that are
wrong for a table. It dissolves three defects at once: a cell with inline markup
refused, a table in a list or quote refused, and the padding itself. +1231 B
gzip, 725 B of it the prose explaining the rewrite. | **unreviewed.** Built by
Claude (Opus 5) 2026-09-09, `7b5bf96`. Five tests each side. | | 20 | Markdown |
Opus (orchestrator) | `layout_leaves` — a manifest list of leaf kinds whose text
gate 3 compares by meaning rather than byte for byte | Row 19's padding rewrites
a leaf, and gate 3 rejected **prettier's own output** on `tables.md` for it:
`('-','-','-')` became eleven dashes, `'a '` became `'a   '`. The gate's own
docstring says a rejection of the reference formatter means the gate is wrong
rather than strict — it is the same failure black's added parentheses produced,
and takes the same remedy, a narrowing declared per language by kind. Outer
whitespace goes; a leaf that is entirely a rule reduces to its anchors, so `:-`
equals `:-----` and does not equal `-----:`. What it stops catching, stated in
the code: a body cell whose content is `--` becoming `---`. | **unreviewed.**
Built by Claude (Opus 5) 2026-09-09, `7b5bf96`. Four unit tests; `check_gate3`
still rejects 782 destructive mutations, markdown's own share up from 32 to 34.
|

| 21 | Markdown | Opus (orchestrator) | `blank_owner` — a manifest-free package
field naming node types whose own source range already carries the blank line
that follows them | Gate 2 run over all 6,615 `*.md` files under `~/w` found 121
documents the formatter is not idempotent on, 65 of which produced output that
would not re-format at all. `indented_code_block`'s range runs past the blank
that terminates it, so `blanks()` reads zero, `blank` forces its cap, and every
pass adds a line. FINDINGS 30 from the other side: `gap_owner` is the gap read
short because it sits deeper than the bound reaches, this is the gap read short
because the node ate it. Declared, never inferred — a rule that reconstructs
rather than slices never emitted those newlines. Walks the last-child spine,
because a block nested in a `list_item` ate the blank just as surely. 121 → 58
not idempotent, refusals 167 → 107. | **unreviewed.** Built by Claude (Opus 5)
2026-09-09, `248e029`. Four tests each side including the undeclared control. |

| 22 | Markdown | Codex | FINDINGS 35: quote separators and trailing list-item
markers use `srcsoft`; headings consume their trailing continuation after `hard`
| No runtime change earned its place. The previous subtree emits the following
paragraph's marker in the correct token order; an unconditional quote separator
then detaches it. Source adjacency already expresses the necessary boundary.
Replacing the item's `blank` with `srcsoft` also separates the two markers
without adding a second gap after nested fences. Added exact repro, nested quote
and list-to-heading cases to `blockquotes.md`; explicit double-format checks in
JS and Rust at 80 and 40 are identical and match Prettier. | **unreviewed.**
Built by Codex, 2026-09-09; package-only change shared by both runtimes. |

| 23 | Markdown | Codex | `whitespace_nodes` package and manifest declarations;
package format 2, with version 1 still supported | FINDINGS 36's empty `section`
is a literal whitespace leaf as well as an item. Consume declared
whitespace-only leaves as source gaps before comment attachment; the next real
item receives one separator and leading trivia none. The subtree must pass
source/range validation; meaningful leaves, interior nodes and injected roots
remain items. No Doc inspection, tree rewriting or new opcode earned a place: 29
opcodes and three sanctioned mutations remain. The manifest declaration is
separately required because gate 3 rejected Prettier's own removal of a leading
section. Added front matter to `sections.md` and a three-blank leading run in
`leading_sections.md`; both explicitly double-format identically in both
runtimes at 80 and 40 and match Prettier. | **unreviewed.** Built by Codex,
2026-09-09. Six mirrored runtime tests, structural and manifest safeguards; all
792 destructive mutations still rejected (782 before the session). JS runtime
+369 B gzip; markdown package +19 B. |

| 24 | Markdown | Codex | Roadmap step 2 (prose wrap), measured and declined —
no opcode, package field or runtime capability added | Switching only the
reference to `--prose-wrap always` drops the unchanged 20-file corpus from 27/32
to 8/32 and makes gate 3 reject 20 of the reference outputs; the live manifest
keeps `preserve`. `fill` selects existing child Docs and cannot split a leaf's
raw text; probing `inline_language()` directly shows it supplies
`emphasis`/`inline_link`/`code_span` but still no word nodes, so an
included-range second pass alone would not be enough either, and Prettier itself
wraps inside a long `emphasis` span. A delimiter-aware splitter living only in
the runtime was considered and declined as a second, partial markdown parser.
Added `prose_wrap.md`, whose reference at both widths is currently identical to
source (the documented limit, not an omission); double-format identical in JS at
80 and 40. | **unreviewed.** Session cut off by a usage limit before it wrote
its own done-note or `corpus/reports/markdown/prose-wrap.md`; the measurements
above were recovered from the working diff and the report, ledger row and
...
2026-08-24) and `gap_owner` (`3555eb2`, 2026-08-24). They are recorded in
FINDINGS 29(b), 24 and 30 and in `corpus/reports/markdown/report.md`, so the
work is documented; the ledger is what is missing it. Reconstructing those rows
from the log is its own task and is not done here.
...
**It is a goal, not a limit, and we expect to go over it** — Dave, 2026-08-21,
at 25,048 B with seven languages still to onboard. This is a **narrower** change
than it sounds, and it does not touch the paragraph above: the attribution
requirement is unchanged, the "is a downloadable-package design still the right
shape" question is unchanged, and nothing here licenses a per-slice lump in the
`gzip Δ` column.

What changes is the **response to going over**. Overrunning does not stop a
merge and does not by itself re-open the architecture; it triggers a
feature-weight audit. Dave's reasoning is that the audit is not answerable yet:
_"it's too hard to see at this point because we haven't done enough languages."_
A capability that looks expensive against nine languages may be carrying four of
the next seven, and there is no way to tell from here.

So the plan is to let the total run over while coverage grows, then ask of every
row in table 1: **what is this still buying?** Two things make that answerable
later and not now:

- The `gzip Δ` column is a **cost at the time it landed**, and every row has
  one. The missing half is current benefit, and it is measurable the same way
  the costs were: remove the capability, re-score every language, count the
  pairs that stop agreeing. `fill` (+365 B) and the `group` sub-width cap (+228
  B) were both measured that way when they landed, against fewer languages than
  we now have.
- Reductions exist and are not all painful. The first one arrived unlooked-for
  on 2026-08-20: `token_tree`'s three enumerated guards collapsed into one `all`
  predicate over the whole child list, which was **22 B smaller**, more general,
  and fixed a live refusal on ordinary Rust. It was found by writing a probe,
  not by hunting for bytes — which is itself the argument for more coverage
  first.

Do not pre-emptively trim to stay under the number, and do not treat the number
as spent once it is crossed. Report the total in every stage report exactly as
now: the figure is the instrument, and turning it off is how the accounting
...
**`package-bug` is a hard merge failure** — Dave, 2026-08-21. It joins a stale
review in `score.py`'s `disqualified`; the scorer exits non-zero and prints
`<- hard failure` next to the count. Both are claims that something is wrong
_now_, and neither stopped a merge before, which made `package-bug` the cheaper
verdict to write whenever a reviewer was unsure — the exact opposite of what the
verdict vocabulary is for. `design-limit` and `house-rule` say _we chose not
to_; `package-bug` says _we could have and did not_, and a bar that never fires
teaches reviewers to reach for the honest-sounding label with no consequences.

The counter-argument, on the decisions page and worth keeping visible: a hard
bar creates pressure to relabel a genuine defect as a design limit. The
counterweight already exists on the other side — the stage-D brief tells
reviewers to take two or three _we chose not to_ verdicts and check whether the
package could in fact have done it. Nothing was checking the merge bar.

**An unreviewed divergence is deliberately not a hard failure.** It is measured
against stage D's 70% per-width coverage floor, because a corpus that outruns
its review is a scheduling problem, not a false claim. Today every merged
language is at 0 stale, 0 unreviewed and 0 `package-bug`, so this change costs
nothing to adopt and would have cost a re-run at three separate points in rounds
2 and 3.
...
after `{` stays expanded even when it fits flat. The alternative, `collapse`, is
what a plain width `group` implements, so the naive package is the wrong one.
Dave's call is to match the default rather than declare the divergence. The
runtime already has `srcline` / `srcsoft` / `srctrail` for exactly this, so this
is a stage-C instruction, not a findings entry. What makes it dangerous is that
a preserved break and a width break are the same bytes: a package that gets it
wrong passes the whole corpus and diverges on real input.
...
| 2026-08-21 | `review-brief.md`, `WORKFLOW.md`      | `package-bug` fails the scorer. Both files now say so, and say which way the resulting pressure runs: the verdict is not the safe one to write, and relabelling a real defect `design-limit` to clear the bar is the worse failure                                                                                                   | Dave's standing decision above. The vocabulary already distinguished _we could not_ from _we chose not to_; nothing enforced the consequence, so the honest-sounding label was also the free one                                                                                          |
| 2026-08-21 | `corpus-brief.md`                     | The fake stdin filename is spelled from the manifest's **`extensions`**, not the language name. `--stdin-filepath` is required only where prettier infers the parser from the filename; `--parser` does the same job without opening `.editorconfig` discovery                                                                       | XML and TypeScript stage B, independently. `x.{{LANG}}` is wrong for five of ten roster languages and prettier rejects `x.typescript` outright; the old text asserted `--stdin-filepath` was "genuinely required for prettier"                                                            |
| 2026-08-21 | `corpus-brief.md`                     | How to pin a **prettier plugin** reference: `--plugin @scope/name` does not resolve under npx, the plugin entry-point path relative to the resolved binary does, and ground truth must be proved from a cold `npm_config_cache`                                                                                                      | XML stage B — the first plugin-based reference on the roster, and nothing in the template covered it                                                                                                                                                                                      |
| 2026-08-21 | `corpus-brief.md`                     | `transparent_wrappers` gains its **second shape**: a unary wrapper a grammar inserts for a leading operator that appears only on break, with the arity proof required by parsing both forms                                                                                                                                          | TypeScript stage B — `union_type` is sound for the one-named-child reason, and the paragraph's mental model was paren-only, which would have sent the next such language to a `gate3` override                                                                                            |
| 2026-08-21 | `corpus-brief.md`                     | The reference's **option table is a command producing an artefact**, not a topic: dump it, go through it item by item, and flag every default that makes layout depend on the input's line breaks rather than width — naming `srcline`/`srcsoft`/`srctrail`                                                                          | Markdown and HTML stage B, independently. It was the one required item both builders skipped, having seen the behaviour in their own output; `embeddedLanguageFormatting` and `htmlWhitespaceSensitivity` both went unrecorded, and `off`/`ignore` are exactly what a naive package emits |
...
| 2026-08-22 | `review-brief.md`                     | **`fixed` languages need a different check.** Check 4 (bisect the default) does not apply; check 7 now says _prove the width is inert_, with the two shapes round 5 used -- Haskell's six rejected flags plus inertness in both directions, Scheme's `fill-column` 40 vs 200 on one line.                                            |
...
| 2026-08-22 | `review-brief.md`                     | **A named rewrite is not an enumerated one.** For every `[incomparable]` construct, ask whether the reference reorders, deletes, merges or renames, and record the negatives. ormolu's import block is sorting _and_ collapsing -- the ktfmt precedent in a second formatter.                                                        |
| 2026-08-22 | `corpus-brief.md`                     | **The report-to-corpus direction becomes a builder step.** The normalisation list is now a table with a _forcing file_ column and an empty cell where none exists. All four round-5 defects were this shape and none moved a count, so nothing else could catch them. Also: run the markdown formatter before committing the report. |
...
**−1,601 B**: the ~2,000 B of quote-aware scanners every named mode had to carry
are gone, and what replaces them is ~992 B of language-independent tabwriter.
Rust alignment is now a handful of package cells and no runtime at all.

Two process notes worth more than the number:

- **The A/B never ran.** agy was to build a second implementation head-to-head;
  it hit a quota wall. The substitute was _independent verification of one
  implementation_ rather than a second implementation — re-running the
  measurements by hand, and set-diffing the GOROOT probe by path. That is
  cheaper than a second spike and caught what a second spike would not have: the
  builder's own real-world claim rested on a 16-file sample.
...
this round is by _what the slice needs_, not by what is available:
...
builder lane has been Claude at all.** Grok is still at a 402 with no reset
hour; codex is needed for stage D on two held branches and on whatever Claude
builds. Five languages sat at `B+` with no builder, which is a stalled pipeline,
not a lane preference.
...
the two held branches without further thought — TypeScript to codex (grok built
it), HTML to an Opus subagent (codex-Sol built it and cannot review its own
family), which is the "central changes to `main`" carve-out being used for what
it was reserved for: HTML's 25 lines of `DESIGN.md`.

The cost being watched: **Claude is uncalibrated as a builder**, the same way
every agent was on its first slice, and the orchestrator is also the builder
this round, so the "the orchestrator does not read the code" property is
suspended. That is why only two of the five are in flight. Codex's first stage-D
verdict on a Claude-built package is the calibration datum; the other three are
held for it.
...
- **`harness/score.py` counted a tab as one column — now fixed.**
  `overflow_lines` uses `len(line)` with no `expandtabs`, so for a tab-indenting
  reference every leading tab under-counts by seven. Inert right now — Go's raw
  and expanded counts both come to 6, Scheme's both to 0 — but Scheme has **11
  of 15** reference files containing a tab, so the first Scheme package that
  overflows will be measured wrong. Found by Scheme's stage B, which named it
  and left it alone as instructed. Fixed at merge time instead of carried:
  `overflow_lines` now measures `line.expandtabs(TAB_WIDTH)` with
  `TAB_WIDTH = 8`, a module constant rather than a manifest field because both
  tab-indenting references on the roster are 8-column tools and none disagrees.
  Two unit tests, one for tabs and one confirming spaces are unaffected. Merging
  tab-indented Scheme onto main while the measurement is known-wrong is worse
  than a one-line fix.
- **Paren transparency has a narrow hole in Haskell.** `g (do x; y)` elides to
  ...
  than fixed. It belongs in `FINDINGS.md` as a sibling of entry 13's "elision
  only fires on exactly one named child", and it is not written there yet --
  **written up as FINDINGS 25**, 2026-08-22. Found by Haskell's stage B, which
  also measured the sound half: of nine paren-drop attacks, six are rejected,
  including every load-bearing one (application spine, precedence,
  associativity, negative literal, type arrow, lambda argument).
...
This is worth recording because of _where_ the bad sentence was: in the
**verification** section, which is the section the trust contract exists to rely
on. The contract says the main thread does not re-run the gates. A verification
section that contains one sentence describing work that was never done is a
signal directly contradicting the claim, which is the documented exception — so
`./test.sh` was re-run on the slice. **It passed, exit 0**, and the injection
probes hold, including the quoted-fence limitation reproducing permanently.
...
**grok, on the same brief for Rust, delivered a complete stage C**: a package, a
runtime fix with its own case and a measured +299 B, a report, a `score.json`,
...
- **A model that passes a bounded task can still fail an open-ended one.** Lane
  assignment should follow task _shape_, not a single quality ranking. On this
...
run where a _negative_ instruction was routed around rather than obeyed (the
...
6m55s, `do_epoll_wait` with **zero open sockets**, no commit. Not the "never
started" signature (it had written 146 KB); a hang after real work. Killed at 36
min.
