...
There are 17 valid Rust source files. `gen_trees.py` reported no `ERROR` or
`MISSING` nodes.
...
- `leading_pipes.rs` — a match whose every arm carries a leading `|`, which
  rustfmt deletes. A token deletion linearity forbids a package from reproducing,
  so the file is declared `[incomparable]` rather than omitting the construct.
...
**Restated for 17 files after `leading_pipes.rs` was added**, from
`./harness/corpus_stats.py --language rust`:

```text
rust  --  17 files, vs rustfmt 1.9.0
  incomparable         1  (gated; out of the agreement denominator)
  reference changes    14/17 at some width   (@100 7/17  @60 13/17)
  differs by width     11/17
  carries a comment    17/17
  reference overflow   @100 0  @60 39
  NOTE 3 file(s) byte-identical input to output at every width
```

The one incomparable file is `leading_pipes.rs`; it sits outside the agreement
denominator and skips only the "reference must pass gate 3" assertion. The
overflow count and the three byte-identical files (`enums.rs`, `sequences.rs`,
`traits.rs`) are unmoved from the 16-file corpus.
...
Reviewer: Claude Opus 5, resumed after a session cutoff. Verdict **pass with
fixes applied**. Every number below was re-run by the reviewer rather than read
out of the sections above; the section "Completed after the session cutoff"
marks the checks the first pass had not reached.
...
- **The reference output is rustfmt's.** Six files regenerated by hand at both
  ...
  across all 34 (17 files × 2 widths).
...
- **`widths = [100, 60]` is rustfmt's own default, bisected.** A `let a + b;`
  ...
  17 corpus files is byte-identical to the committed `@100` reference. The
...
- **`reference_width = "flag"` is honest**; the two widths produce different
  output on 11 of 17 files.
- **No `ERROR` and no `MISSING` node** in any of the 17 trees, rechecked after
  the stage-B edits.
...
| `struct_lit_single_line = true`      | 3/16 (when off)       | keeping a small struct literal flat         |
| `use_small_heuristics = Default`     | Max 2/16, Off 4/16    | the master switch over the four sub-widths  |

`struct_lit_single_line` is the one row whose default is already **on**: the
header "turning it on changes" does not apply. The 3/16 is what turning it
_off_ costs — small struct literals that rustfmt keeps flat by default.
...
### A second exclusion: a deletion, not a reordering — now declared, not omitted
...
as import sorting. Unlike import sorting it cannot be dodged by writing the
canonical form — there is no canonical form with a leading pipe, since rustfmt
always removes it — so the construct is either absent or incomparable.

The first pass recorded this as "silently unmeasured" and deferred the decision
("whether excluded constructs get probe files or only a written record … is
Dave's call"). That deferral was stale: FINDINGS 4 is already **decided —
option 2, probe files**. The construct also changes the _named_ tree, not just an
anonymous token: a leading `|` turns the pattern into an `or_pattern`, so
rustfmt's deletion renames `or_pattern` to `integer_literal` and gate 3 rejects
the reference's own output. That is precisely what `[incomparable]` exists for.

**Fixed:** `leading_pipes.rs` is a match whose every arm carries a leading pipe,
declared `[incomparable]` in the manifest. It skips only the "reference must
pass gate 3" assertion and sits outside the agreement denominator; it still
counts for gates 0–3, so a future package must still format it without
destroying it.
...
That is the whole of it. Import sorting stays dodged rather than declared — see
the template delta for why that is the right call for reordering specifically.
...
no longer exists; the live number is in the 17-file block above it.

### Completed after the session cutoff

The first pass was killed mid-write and had not reached the three checks after
"files touched outside" — 9, 10 and 11 — nor the verdict and template delta.
They are here. Anything marked "re-run" below was computed fresh in this pass.

#### Check 9 — the reference's off-by-default options (re-run, held)

The table above was written by the first pass; I re-derived it by setting each
option against the whole corpus at width 100 and diffing the default run. Every
one of the eleven rows matches: `wrap_comments` 1/16, `format_strings` 1/16,
`struct_field_align_threshold` 4/16, `enum_discrim_align_threshold` 0/16,
`normalize_comments` 1/16, `imports_granularity` 1/16, `group_imports` 1/16,
`where_single_line` 1/16, `match_block_trailing_comma` 3/16,
`struct_lit_single_line` 3/16 (off), `use_small_heuristics` Max 2/16 / Off 4/16.
The one correction made is the `struct_lit_single_line` framing above. The
zero-effect list is recorded as unobservable, a weaker claim I did not re-walk.

#### Check 10 — incomparable files are dedicated (fixed, see above)

Before this pass the manifest declared no `[incomparable]` entry while the
report named two excluded behaviours — import sorting (dodged) and leading-pipe
deletion (omitted). The deletion is now a dedicated `leading_pipes.rs`. Import
sorting deliberately stays dodged rather than declared: see the template delta.

#### Check 11 — report-to-corpus audit (re-run, held)

`./harness/corpus_stats.py --language rust` prints exactly what the report's
17-file block says: 1 incomparable, 14/17 changed, 11/17 differ by width, 17/17
carry a comment, overflow @100 0 / @60 39. The three byte-identical files are
`enums.rs`, `sequences.rs`, `traits.rs`, confirmed by `cmp`. Node kinds are 157
distinct, re-counted from the committed trees — 156 before `leading_pipes.rs`
added `or_pattern`.

#### `widths.rs`, verified independently

The orchestrator wrote this file and asked that it not be taken on trust. I
measured the flat widths myself rather than reading the comments: the struct
body is 36 columns against `struct_lit_width` 18 (line 65); the chain 61 against
`chain_width` 60 (line 79); the call arguments 64 against `fn_call_width` 60
(line 91); the array 63 against `array_width` 60 (line 82). All four fit
`max_width=100` flat and are broken anyway, which the committed `@100` output
confirms. The `under_every_threshold` controls measure 7 / 37 / 20 / 21 against
their thresholds and stay flat in the `@100` output. `aligned_arms` aligns all
three at 100 and splits at 60 exactly as the comment claims. The notes are now
correct; the orchestrator's earlier wrong sizes are gone.

#### A shared-file edit, and why

Adding the `[incomparable]` entry surfaced `harness/test_manifest.py`'s
`test_merged_languages_declare_none`, which asserts that no loaded manifest
declares an incomparable file — the snapshot the field's own landing slice left
behind ("this slice adds the field, not the files"). Rust is the first language
to use the field, so the test now exempts `rust` and pins that its only entry is
`leading_pipes.rs`. One-line scope; stated here because it is outside `corpus/`
and `harness/languages/`.

#### Verdict

**pass with fixes applied.** The fixes are `leading_pipes.rs` plus its manifest
entry (check 10), the `struct_lit_single_line` framing correction, and the
`test_manifest.py` exemption. `./test.sh` exits 0 with zero warnings and
`./harness/score.py .` passes.

#### Template delta (`templates/corpus-brief.md`)

1. **Distinguish a dodgeable reordering from a non-dodgeable deletion.** The
   brief's `[incomparable]` section lists "gofmt sorting imports" as a canonical
   example and says "do not leave the construct out of the corpus", but the
   merged Go corpus dodges import sorting (`imports.go` is pre-sorted) and that
   was accepted. A reordering can be dodged — pre-sorted imports still exercise
   the comparable use-group *layout*, and the reordering itself is permanently
   out of scope (FINDINGS 14). A deletion cannot: rustfmt's leading-`|` removal
   has no comparable form, so the construct is either absent or declared. The
   brief should say "dodge a reordering, declare a deletion or rewrite". The
   confusion is what made the first pass defer on the leading-pipe case by
   reading FINDINGS 4 as still open when it is decided (option 2, probe files).
2. **Probe the reference's width thresholds, not only `max_width`.** rustfmt
   breaks constructs that fit `max_width` because it carries nine sub-widths
   (FINDINGS 17). The corpus as built exercised only `struct_lit_width`
   incidentally (`nesting.rs`), leaving `chain_width` and `fn_call_width` — the
   two that with `struct_lit_width` decide 44.8% of real rustfmt-clean files —
   unexercised, and the orchestrator had to add `widths.rs` after stage A. The
   brief should ask the builder to discover and probe the reference's own width
   thresholds beyond the flag the manifest records, so entry 17's method runs at
   stage A rather than as a stage-B rescue.
