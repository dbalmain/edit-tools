...
# dependencies = []
# ///
"""Pin `<wctype.h>` classification and case mapping as data, so a port cannot inherit a host.
...
## Case mapping is the same problem, and classification was only half of it

Classification is not the only libc call a scanner makes. tree-sitter-html
pushes `towupper(lexer->lookahead)` into every tag name it stores, and matches
`towupper` against its `</SCRIPT` / `</STYLE` raw-text delimiters -- so which
characters are "the same tag name" is a property of the host in exactly the way
whitespace is. `maps` carries that: `[lo, hi, delta, ...]`, inclusive runs of
constant delta, identity everywhere else.

That representation is what makes it affordable. glibc moves **1,477 code
points**, and they compress to **690 triples**; as branches in a program it
would be absurd, as a table it is under a kilobyte. It is why the VM gained a
`MAP` instruction rather than html's port gaining an approximation of
`towupper` -- see `docs/scanner-vm.md`.
...
# The same idea for the case map. Each row discriminates against a host that
# maps only ASCII (the C locale leaves U+00E9 alone) or that lacks glibc's
# special cases (U+017F LATIN SMALL LETTER LONG S upper-cases to plain S).
MAP_CONTROLS = (
    ("upper", 0x0061, 0x0041, "ASCII 'a' -- the positive control"),
    ("upper", 0x0041, 0x0041, "'A' -- identity, the negative control"),
    ("upper", 0x00E9, 0x00C9, "e-acute -- identity under the C locale"),
    ("upper", 0x017F, 0x0053, "LATIN SMALL LETTER LONG S -- not a simple delta"),
)
...
def generate() -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for locale in LOCALES:
            ...
            if primary is None:
                primary = result
            elif (result["classes"], result["maps"]) != (
                primary["classes"], primary["maps"]
            ):
                differ = sorted(
                    k
                    for section in ("classes", "maps")
                    for k in primary[section]
                    if primary[section][k] != result[section][k]
                )
                raise CtypeError(
    ...
    return {
        "note": (
            ...
            "<wctype.h> classification and case mapping under UTF-8, which is "
            "what gen_trees.py freezes the corpus with. Classes are inclusive "
            "[lo, hi, ...] over code points; maps are [lo, hi, delta, ...], "
            "identity outside the listed runs."
        ),
...
def apply_map(triples: list[int], cp: int) -> int:
    """What the VM's MAP instruction computes: delta if in a run, else identity."""
    for i in range(0, len(triples), 3):
        if triples[i] <= cp <= triples[i + 1]:
            return cp + triples[i + 2]
    return cp


def controls(tables: dict) -> list[str]:
    """Every documented measurement these tables fail to reproduce."""
    problems = []
    for name, cp, expected, why in CONTROLS:
        ranges = tables["classes"][name]
        got = any(ranges[i] <= cp <= ranges[i + 1] for i in range(0, len(ranges), 2))
        if got != expected:
            problems.append(f"{name}(U+{cp:04X}) is {got}, expected {expected} -- {why}")
    for name, cp, expected, why in MAP_CONTROLS:
        got = apply_map(tables["maps"][name], cp)
        if got != expected:
            problems.append(
                f"{name}(U+{cp:04X}) is U+{got:04X}, expected U+{expected:04X} -- {why}"
            )
    return problems
