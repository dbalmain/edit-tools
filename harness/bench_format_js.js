...
const packages = new Map([[tree.language, pkg]]);
const width = Number(widthArg);
const iterations = Number(iterationsArg);

for (let index = 0; index < 10; index++) format(tree, packages, width);
const started = process.hrtime.bigint();
for (let index = 0; index < iterations; index++) format(tree, packages, width);
