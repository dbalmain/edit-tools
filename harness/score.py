...
class Report:
    ...
    # Languages excluded from every gate, mapped to why: own package missing,
    # or a guest they format is pending.
...
def reference_agreement(
    ...
    denominator: a language with 15 comparable files and 3 incomparable ones
    scores n/15, not n/18. The count is per file, not per width — a file is
    excluded at every width.
    ...
    for tree_path, m in trees:
        ...
        if file in m.incomparable:
            entry["excluded"] += 1
            entry["excluded_files"].append(
                {"file": file, "reason": m.incomparable[file]}
            )
            for width in m.widths:
                language_seen.add(f"{m.name}/{file}@{width}")
            continue
        for width in m.widths:
            case = f"{file}@{width}"
...
def main() -> int:
    ...
    if not scored:
        names = ", ".join(
            f"{n} ({pending[n]})" for n in sorted(pending)
        )
    ...
    rep.pending = {n: pending[n] for n in sorted(pending)}
    ...
    if rep.pending:
        for name, reason in rep.pending.items():
            print(f"  awaiting package   {name} ({reason})")
        print()
